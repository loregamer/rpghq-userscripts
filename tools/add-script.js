...
async function createScript() {
  ...
  const category =
    (await prompt('Enter Category (default: "General"): ')) || "General";
  const enabledByDefault =
    (await prompt("Enable by default? (y/n): ")).toLowerCase() === "y";
...
`;
  ...
  if (importSection !== -1) {
    ...
    mainJsContent =
      mainJsContent.slice(0, nextLineAfterImports) +
      importLine +
      mainJsContent.slice(nextLineAfterImports);
    ...
    if (scriptModulesSection !== -1) {
      ...
      const moduleMapLine = `\n  ${scriptId}: ${scriptId},`;
      mainJsContent =
        mainJsContent.slice(0, modulesMappingEnd) +
        moduleMapLine +
        mainJsContent.slice(modulesMappingEnd);
