...
(function () {
  ...
  // ---------------------------------------------------------------------
  // 0) Separate Storage for Ghosted Users
  // ---------------------------------------------------------------------
  // Fully ghosted users (posts will be hidden)
  const ghostedUsers = GM_getValue("ignoredUsers", {});
  // Semighosted users (posts/rows will be highlighted with a different color)
  const semighostedUsers = GM_getValue("semighostedUsers", {});

  // Other stored values remain unchanged.
  ...
  // ---------------------------------------------------------------------
  // 1) INITIAL STYLES (CSS)
  // ---------------------------------------------------------------------
  const mainStyle = document.createElement("style");
  mainStyle.textContent = `
    /* Spinner styling for containers that are not yet processed */
    ...
    /* Fully ghosted elements styling */
    .ghosted-row {
      display: none !important;
    }
    .ghosted-row.show {
    ...
    /* Semighosted elements styling */
    .semighosted-post {
      /* Visible posts highlighted with a blue-ish tint */
      background-color: rgba(0, 170, 255, 0.1) !important;
      border: 1px dashed rgba(0, 170, 255, 0.4);
    }
    .semighosted-row {
      /* Rows for semighosted users get a blue-ish background */
      background-color: rgba(0, 170, 255, 0.1) !important;
    }
    .semighosted-mention {
      /* For @mentions in posts by semighosted users */
      background-color: rgba(0, 170, 255, 0.1) !important;
    }
  ...
  // ---------------------------------------------------------------------
  // 3) GHOST/SEMIGHOST FUNCTIONS & HELPERS
  // ---------------------------------------------------------------------
  ...
  // Instead of a single ignoredUsers, we now use two storages.
  function toggleUserGhost(userId, username) {
    const cleanedUsername = cleanUsername(username);
    if (ghostedUsers.hasOwnProperty(userId)) {
      // If fully ghosted, unghost user.
      delete ghostedUsers[userId];
      GM_setValue("ignoredUsers", ghostedUsers);
    } else if (semighostedUsers.hasOwnProperty(userId)) {
      // If semighosted, promote to fully ghosted.
      delete semighostedUsers[userId];
      ghostedUsers[userId] = cleanedUsername.toLowerCase();
      GM_setValue("semighostedUsers", semighostedUsers);
      GM_setValue("ignoredUsers", ghostedUsers);
    } else {
      // If not ghosted, add to semighosted.
      semighostedUsers[userId] = cleanedUsername.toLowerCase();
      GM_setValue("semighostedUsers", semighostedUsers);
    }
  }

  // Returns "ghost", "semighost", or null.
  function getGhostState(userIdOrUsername) {
    if (ghostedUsers.hasOwnProperty(userIdOrUsername)) return "ghost";
    if (semighostedUsers.hasOwnProperty(userIdOrUsername)) return "semighost";
    const cleaned = userIdOrUsername.toLowerCase();
    for (const key in ghostedUsers) {
      if (ghostedUsers[key] === cleaned) return "ghost";
    }
    for (const key in semighostedUsers) {
      if (semighostedUsers[key] === cleaned) return "semighost";
    }
    return null;
  }

  function isFullyGhosted(user) {
    return getGhostState(user) === "ghost";
  }
  function isSemighosted(user) {
    return getGhostState(user) === "semighost";
  }
  ...
  // In this version, only fully ghosted users cause posts/rows to be hidden.
  // Semighosted users will have a distinct highlighting.
  function postContentContainsGhostedMention(post) {
    const contentDiv = post.querySelector(".content");
    if (!contentDiv) return null;
    const postText = contentDiv.textContent.toLowerCase();
    let foundState = null;
    for (const uid in ghostedUsers) {
      if (postText.includes(ghostedUsers[uid])) return "ghost";
    }
    for (const uid in semighostedUsers) {
      if (postText.includes(semighostedUsers[uid])) foundState = "semighost";
    }
    return foundState;
  }
  ...
  function cleanupTopicAuthor(element) {
    ...
    if ((userId && isFullyGhosted(userId)) || isFullyGhosted(username)) {
  ...
  function hideTopicRow(element) {
    ...
    if (recentTopicLi) {
      const forumLinks = recentTopicLi.querySelectorAll(
        "a.username, a.username-coloured"
      );
      let state = null;
      forumLinks.forEach((link) => {
        const s = getGhostState(link.textContent.trim());
        if (s === "ghost") state = "ghost";
        else if (!state && s === "semighost") state = "semighost";
      });
      if (state === "ghost") {
        recentTopicLi.classList.add("ghosted-row", "ghosted-by-author");
      } else if (state === "semighost") {
        recentTopicLi.classList.add("semighosted-row");
      }
      return;
    }
  ...
  async function processLastPost(element) {
    ...
    if (row) {
      ...
      for (const link of authorLinks) {
        if (isFullyGhosted(link.textContent.trim())) {
          hideTopicRow(row);
      ...
      const hasGhostedClass = Array.from(row.classList).some(
        (cls) =>
          ...
          isFullyGhosted(cls.replace("author-name-", ""))
      );
      if (hasGhostedClass) {
        hideTopicRow(row);
        element.classList.add("content-processed");
    ...
    if (
      ...
      if (link) {
        const pid = link.href.match(/[#&]p=?(\d+)/)?.[1];
        if (pid) {
          if (userEl && isFullyGhosted(userEl.textContent.trim())) {
            ...
            try {
              ...
              if (
                !content ||
                postContentContainsGhostedMention(element) === "ghost"
              ) {
                hideTopicRow(element);
              }
            } catch (err) {
  ...
  async function processCPListNotification(item) {
    ...
    if (isQuoteOrMention && isFullyGhosted(firstUsername)) {
    ...
    const nonIgnored = usernames.filter((u) => !isFullyGhosted(u));
    const ghosted = usernames.filter((u) => isFullyGhosted(u));
  ...
  async function processNotification(item) {
    ...
    const firstUsername = usernames[0];
    if (isFullyGhosted(firstUsername)) {
  ...
  function processBlockquotesInPost(post) {
    ...
    if (topLevelBlockquotes.length === 1) {
      ...
      if (anchor && isFullyGhosted(anchor.textContent.trim())) {
    ...
    allBlockquotes.forEach((bq) => {
      ...
      if (!anchor) return;
      if (isFullyGhosted(anchor.textContent.trim())) {
  ...
  function processPost(post) {
    ...
    if (usernameEl) {
      const state = getGhostState(usernameEl.textContent.trim());
      if (state === "ghost") {
        hideIt = true;
      } else if (state === "semighost") {
        post.classList.add("semighosted-post");
      }
    }
    const mentionState = postContentContainsGhostedMention(post);
    if (mentionState === "ghost") {
      hideIt = true;
      post.classList.add("ghosted-by-content");
    } else if (mentionState === "semighost") {
      post.classList.add("semighosted-mention");
    }
    if (hideIt) {
  ...
  function processTopicPoster(poster) {
    ...
    if (isFullyGhosted(usernameEl.textContent.trim())) {
  ...
  function addGhostButtonsIfOnProfile() {
    ...
    let titleText = Array.from(memberlistTitle.childNodes)
    ...
    function refreshGhostBtn() {
      const state = getGhostState(userId);
      if (state === "semighost") {
        ghostBtn.innerHTML = '<i class="fa fa-user-secret"></i> Ghost User';
        ghostBtn.title = "Promote to full ghost (posts will be hidden)";
      } else if (state === "ghost") {
        ghostBtn.innerHTML = '<i class="fa fa-user-times"></i> Unghost User';
        ghostBtn.title = "Stop ignoring this user";
      } else {
        ghostBtn.innerHTML = '<i class="fa fa-adjust"></i> Semighost User';
        ghostBtn.title = "Semighost this user (highlight their posts)";
      }
    }
    refreshGhostBtn();
  ...
  function toggleGhostedPosts() {
    ...
    const semighostedRows = document.querySelectorAll(".semighosted-row");

    const hasGhostedContent =
      ghostedPosts.length > 0 ||
      ghostedQuotes.length > 0 ||
      ghostedRows.length > 0 ||
      semighostedRows.length > 0;
    ...
    if (showGhostedPosts) {
      const firstShownElement =
        ...
        document.querySelector(".semighosted-row.show");
  ...
  function processOnlineList() {
    ...
    const nonGhostedUsers = userLinks.filter((link) => {
      ...
      return (
        !(userId && isFullyGhosted(userId)) &&
        !isFullyGhosted(link.textContent.trim())
      );
    });
  ...
  // ---------------------------------------------------------------------
  // 10) GHOSTED USERS MANAGEMENT POPUP
  // ---------------------------------------------------------------------
  function showIgnoredUsersPopup() {
    ...
    userList.style.cssText = `
      ...
      margin: 0;
    `;
    // Combine ghosted and semighosted users into one list.
    const ghostEntries = Object.entries(ghostedUsers).map(
      ([userId, username]) => ({ userId, username, state: "ghost" })
    );
    const semighostEntries = Object.entries(semighostedUsers).map(
      ([userId, username]) => ({ userId, username, state: "semighost" })
    );
    const userEntries = ghostEntries.concat(semighostEntries);
    userEntries.sort((a, b) => a.username.localeCompare(b.username));
    userEntries.forEach(({ userId, username, state }) => {
      ...
      stateLabel.textContent = state === "ghost" ? " (Ghost)" : " (Semighost)";
      stateLabel.style.cssText =
        "color: #aaa; font-size: 0.8em; margin-left: 5px;";
      listItem.appendChild(unghostedButton);
      listItem.appendChild(userLink);
      listItem.appendChild(stateLabel);
    ...
    paginationElements.forEach((pagination) => {
      ...
      const visibleMatches = showGhostedPosts
        ...
        : document.querySelectorAll(
            "li.row:not(.ghosted-row):not(.semighosted-row)"
          ).length;
