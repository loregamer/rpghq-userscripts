...
(function () {
  ...
  // After the removeNotificationButtonId function, add our new notification handling code
  function setupNotificationSystem() {
    // Hide the original notification dropdown
    const originalDropdown = document
      .querySelector("#notification_list_button")
      ?.closest(".dropdown-container");
    if (originalDropdown) {
      originalDropdown.style.display = "none";
    }

    // Create our custom notification container
    const customDropdown = document.createElement("li");
    customDropdown.className =
      "dropdown-container dropdown-right rightside dropdown-down";
    customDropdown.setAttribute("data-skip-responsive", "true");

    // Create the button
    const button = document.createElement("a");
    button.href = "#"; // Changed from UCP link to prevent navigation
    button.className = "dropdown-trigger dropdown-toggle";
    button.innerHTML = `
    ...
    dropdownContent.className = "dropdown dropdown-extended notification_list";
      ...
      <div class="dropdown-contents">
        <div class="header">
          ...
          <span class="header_settings">
            ...
            <span id="mark_all_notifications"> • <a href="./ucp.php?i=ucp_notifications&amp;mode=notification_list&amp;mark=all&amp;token=${phpbb.csrf_token}" data-ajax="notification.mark_all_read">Mark all read</a></span>
          </span>
        </div>
        <ul></ul>
        <div class="footer">
    ...
    // Insert it where the original was
    if (originalDropdown) {
      originalDropdown.parentNode.insertBefore(
        customDropdown,
        originalDropdown
      );
    }

    // Set up click handler for the dropdown
    button.addEventListener("click", (e) => {
      e.preventDefault();
      e.stopPropagation();
      ...
      customDropdown.classList.toggle("dropdown-visible", !isVisible);
    });
    ...
    document.addEventListener("click", (e) => {
      if (!customDropdown.contains(e.target)) {
        ...
        customDropdown.classList.remove("dropdown-visible");
      }
    });

    // Set up a mutation observer to watch the original notification list
    ...
    // Start observing the original notification list
    const originalList = document.querySelector("#notification_list ul");
    if (originalList) {
      observer.observe(originalList, { childList: true, subtree: true });
    }

    // Initial processing of notifications
    processNotifications();

    // Handle mark all read functionality
    const markAllLink = dropdownContent.querySelector(
      '[data-ajax="notification.mark_all_read"]'
    );
    if (markAllLink) {
      markAllLink.addEventListener("click", async (e) => {
        e.preventDefault();
        try {
          await fetch(markAllLink.href, {
            method: "GET",
            credentials: "include",
          });
          // Clear notifications after marking all as read
          const customList = dropdownContent.querySelector("ul");
          if (customList) {
  ...
  function processNotifications() {
    ...
    const customDropdown = document.querySelector(
      ".dropdown-container:not([style*='display: none'])"
    );
    if (!originalList || !customDropdown) return;

    const customList = customDropdown.querySelector(".notification_list ul");
    if (!customList) return;

    // Clear our custom list
    customList.innerHTML = "";

    // Get all visible notifications from original list
    const notifications = Array.from(originalList.children).filter(
      (notif) => !notif.style.display || notif.style.display !== "none"
    );

    let visibleCount = 0;

    notifications.forEach((notif) => {
      const notifBlock = notif.querySelector(".notification-block");
      if (!notifBlock) return;

      // Check if this notification is from an ignored user
      const usernames = Array.from(
        notifBlock.querySelectorAll(".username, .username-coloured")
      ).map((el) => el.textContent.trim());

      const hasIgnoredUser = usernames.some((username) =>
        isUserIgnored(username)
      );

      if (!hasIgnoredUser) {
        // If not from ignored user, add to our list
        const clone = notif.cloneNode(true);
        customList.appendChild(clone);
        visibleCount++;

        // Add click handler to mark as read when clicked
        const notifLink = clone.querySelector("a.notification-block");
        if (notifLink) {
          notifLink.addEventListener("click", () => {
            const markReadLink = notif.querySelector("a.mark_read");
            if (markReadLink) {
              fetch(markReadLink.href, {
                method: "GET",
                credentials: "include",
              }).catch(console.error);
            }
    ...
    const badge = customDropdown.querySelector(".badge");
    if (badge) {
