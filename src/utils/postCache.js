...
export function cachePostsFromTopicReview() {
  ...
  posts.forEach((post, index) => {
    ...
    debug(`Post element: ${post.tagName}${post.id ? '#'+post.id : ''} class="${post.className}"`);
    ...
    if (postBody) {
    log(`Found postbody: ${postBody.tagName}${postBody.id ? '#'+postBody.id : ''} class="${postBody.className}"`);
    if (postBody.id) {
    ...
    if (!postId) {
      ...
      log(`Found ${messageDivs.length} message_ divs in review post ${index+1}`);
    ...
    if (!postId) {
      ...
      log(`Found ${anchors.length} anchors with #p in href in review post ${index+1}`);
      if (anchors.length > 0) {
    ...
    if (!postId) {
      log(`Could not find post ID for review post ${index+1} using any method`);
    ...
    log(`Successfully identified post ID ${postId} using ${extractionMethod} method`);
    ...
    const contentDiv = postBody?.querySelector(".content") || post.querySelector(".content");
    const messageDiv = post.querySelector(`#message_${postId}`);
    
    // Log content finding results
    if (contentDiv) {
      log(`Found content div for post ${postId}: ${contentDiv.tagName}${contentDiv.id ? '#'+contentDiv.id : ''} class="${contentDiv.className}"`);
      const contentPreview = contentDiv.innerHTML.substring(0, 100).replace(/\n/g, ' ') + '...';
    ...
    if (messageDiv) {
      log(`Found message div for post ${postId}: ${messageDiv.tagName}#${messageDiv.id}`);
      const messagePreview = messageDiv.textContent.substring(0, 100).replace(/\n/g, ' ') + '...';
    ...
    if (contentDiv || messageDiv) {
      ...
      if (contentDiv) {
        ...
        log(`Cached HTML content for post ${postId} (${contentDiv.innerHTML.length} characters)`);
      }

      if (messageDiv) {
        cacheEntry.bbcode = messageDiv.textContent;
        log(`Cached BBCode content for post ${postId} (${messageDiv.textContent.length} characters)`);
      }
...
export function cachePostsFromViewTopic() {
  ...
  if (posts.length === 0) {
    ...
    const contentArea = document.querySelector("#page-body, .page-body, main, .main-content");
    if (contentArea) {
      log(`Found main content area: ${contentArea.tagName}${contentArea.id ? '#'+contentArea.id : ''} class="${contentArea.className}"`);
  ...
  posts.forEach((post, index) => {
    ...
    if (post.id) {
      ...
      if (postId !== rawId) { // If replacement happened
        ...
        postId = null;
        log(`Post has ID attribute ${rawId} but it doesn't match expected format`);
      }
    } else {
    ...
    if (!postId) {
      ...
      if (postBody) {
        ...
        if (postBody.id) {
          ...
          if (postId !== bodyId) { // If replacement happened
            extractionMethod = "postbody ID";
            log(`Extracted post ID ${postId} from postbody ID: ${bodyId}`);
          } else {
            postId = null;
            log(`Postbody has ID ${bodyId} but it doesn't match expected format`);
          }
        } else {
    ...
    if (!postId) {
      const anchors = post.querySelectorAll("a[href*='#p']");
      log(`Found ${anchors.length} anchors with #p in href in post ${index+1}`);
      if (anchors.length > 0) {
        const anchor = anchors[0];
        const href = anchor.href;
        log(`First anchor href: ${href}`);
    ...
    if (!postId) {
      log(`Could not find post ID for post element ${index+1} using any method`);
      // Log the HTML structure to help diagnose the issue
      const postHTML = post.outerHTML.substring(0, 300) + '...';
      debug(`Post HTML preview: ${postHTML}`);
      return;
    ...
    newPostsCached++;
    log(`Successfully cached post ${postId} (${contentDiv.innerHTML.length} characters)`);
  });
...
if (lastPosts.length === 0) {
  ...
  // Try to determine why by examining page structure
    log("Examining page structure to understand why no lastpost elements were found...");
    const body = document.body;
    const classes = body.className;
    log(`Body classes: ${classes}`);
    
    // Try to find forum listing or topic listing elements
    const forumRows = document.querySelectorAll(".topiclist, .forumlist, .topic-list, .forum-list");
    if (forumRows.length > 0) {
      log(`Found ${forumRows.length} forum/topic list container elements`);
      const firstRow = forumRows[0];
      log(`First forum/topic listing element: ${firstRow.tagName}${firstRow.id ? '#'+firstRow.id : ''} class="${firstRow.className}"`);
      
      // Look for alternative lastpost containers
      const potentialContainers = document.querySelectorAll("[class*='last'], [class*='post']");
      log(`Found ${potentialContainers.length} potential alternative lastpost containers`);
      if (potentialContainers.length > 0) {
        for (let i = 0; i < Math.min(3, potentialContainers.length); i++) {
          const container = potentialContainers[i];
          log(`Potential lastpost container ${i+1}: ${container.tagName}${container.id ? '#'+container.id : ''} class="${container.className}"`);
        }
      }
    } else {
  ...
  lastPosts.forEach((lastPost, index) => {
    ...
    debug(`Lastpost element: ${lastPost.tagName}${lastPost.id ? '#'+lastPost.id : ''} class="${lastPost.className}"`);
    
    // Find any link that might contain a topic reference
    const linkElements = lastPost.querySelectorAll(
      "a[href*='viewtopic'], a[href*='t=']",
    );
    log(`Found ${linkElements.length} potential topic links in lastpost element ${index+1}`);
    
    if (linkElements.length === 0) {
      log(`No topic links found in lastpost element ${index+1}, examining HTML structure...`);
    ...
    for (const linkElement of linkElements) {
      ...
      if (!topicIdMatch) {
        debug(`Link href ${href} doesn't contain a topic ID in the expected format`);
        continue;
      }
      ...
      if (existingTopic) {
        log(`Updating existing cached topic ${topicId}, last seen ${now - existingTopic.lastSeen}ms ago`);
      } else {
  ...
  if (newTopicsCached > 0) {
    log(`Successfully cached ${newTopicsCached} topic IDs from ${lastPosts.length} lastpost elements`);
    gmSetValue(CACHE_KEY, cachedPosts);
    ...
    log("This could be due to no new topics found or all topics already being cached");
  }
}
