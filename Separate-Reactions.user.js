...
(function () {
  ...
  // GM setting for displaying avatar circles
  let displayAvatarCircles = GM_getValue("displayAvatarCircles", false);

  // Register menu command to toggle avatar circles
  GM_registerMenuCommand("Toggle Avatar Circles", toggleAvatarCircles);

  function toggleAvatarCircles() {
    displayAvatarCircles = !displayAvatarCircles;
    GM_setValue("displayAvatarCircles", displayAvatarCircles);
    alert(
      `Avatar circles are now ${
        displayAvatarCircles ? "enabled" : "disabled"
        ...
        <div class="reaction-score-list" data-post-id="${postId}" data-title="Reactions">
            <div class="list-scores" style="display: flex; flex-wrap: wrap; gap: 4px;">
                ${reactions
                  .map(
                    (reaction) => `
                    <div class="reaction-group" style="display: flex; align-items: center; background-color: rgba(255,255,255,0.1); border-radius: 8px; padding: 2px 6px; position: relative; margin-bottom: ${
                      displayAvatarCircles ? "8px" : "0"
                    };">
