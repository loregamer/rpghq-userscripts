...
(function () {
  ...
  const ExecutionFramework = {
    ...
    execute: function (phase, eventName = null) {
      if (phase === ExecutionPhase.CUSTOM_EVENT) {
        if (
          !eventName ||
          !this.queues[ExecutionPhase.CUSTOM_EVENT][eventName]
        ) {
          return;
        }
  ...
  const ScriptManager = {
    ...
    executeScript: function (scriptId, scriptData) {
      try {
        ...
        if (!code) {
          console.error(
            `Script code not found for ${scriptName} (${scriptId})`
          );
          return;
        }
        const executionPhase =
          scriptData.executionPhase || ExecutionPhase.DOCUMENT_READY;
        ...
        console.error(
          `Error registering script ${scriptData.name || scriptId}:`,
          e
        );
        return false;
      }
  ...
  const UI = {
    ...
    renderGallery: function (scripts) {
      ...
      const content = document.getElementById("mod-manager-content");
      content.innerHTML =
        filterHtml + `<div id="mod-gallery-items">${galleryItemsHtml}</div>`;
      document
        .getElementById("gallery-filter-button")
        .addEventListener("click", function () {
          const selectedCategory = document.getElementById(
            "gallery-category-filter"
          ).value;
          const searchQuery = document
            .getElementById("gallery-search-filter")
            .value.toLowerCase();
          ...
          const newGalleryHtml = UI.renderGalleryItems(
            filteredScripts,
            installedScripts
          );
          document.getElementById("mod-gallery-items").innerHTML =
            newGalleryHtml;
    ...
    renderGalleryItems: function (scripts, installedScripts) {
      ...
      scripts.forEach((script) => {
        ...
        html += `
                    <div class="mod-gallery-item" data-script-id="${
                      script.id
                        ...
                        <div class="mod-gallery-item-image">
                            ...
                            ${
                              isInstalled
                                ? '<div class="installed-banner"><i class="fa fa-check"></i> Installed</div>'
                                : ""
                            }
                        </div>
                        <div class="mod-gallery-item-title">${script.name}</div>
                        <div class="mod-gallery-item-version">v${
                          script.version
                        }</div>
    ...
    addGalleryEventListeners: function () {
        ...
        btn.addEventListener("click", function () {
          ...
          const modName =
            document.querySelector(
              `[data-script-id="${scriptId}"] .mod-gallery-item-title`
            )?.innerText || "this mod";
      ...
      document.querySelectorAll(".mod-settings").forEach((btn) => {
        btn.addEventListener("click", function () {
          const scriptId = this.dataset.scriptId;
          const script = ScriptManager.manifest.scripts.find(
            (s) => s.id === scriptId
          );
          if (script) {
      ...
      document
        ...
        .addEventListener("click", function () {
          const newSettings = {
            ...
            notifications: document.getElementById("global-notifications")
              .checked,
            itemsPerPage: parseInt(
              document.getElementById("global-itemsPerPage").value
            ),
          };
    ...
    showModSettingsView: function (script) {
      ...
      if (script.settings && script.settings.length > 0) {
        script.settings.forEach((setting) => {
          settings[setting.id] =
            scriptData.settings && scriptData.settings[setting.id] !== undefined
        ...
        script.settings.forEach((setting) => {
          ...
          if (setting.type === "boolean") {
            ...
            html += `<div class="mod-setting-row" style="display: flex; align-items: center; gap: 10px;">
                                    <span id="mod-setting-${script.id}-${
              setting.id
            }" class="toggle-switch" data-value="${value}" style="cursor:pointer;">
                                        ${
                                          value
                                    ...
                                    <label for="mod-setting-${script.id}-${
              setting.id
            }" style="margin:0;">${setting.label}</label>
                                 </div>`;
      ...
      document
        .getElementById("back-to-installed")
        .addEventListener("click", function () {
          UI.switchTab("installed");
        });
    },
    ...
    addInstalledEventListeners: function () {
      ...
      document.querySelectorAll(".mod-uninstall").forEach((btn) => {
        btn.addEventListener("click", function () {
          ...
          const modName =
            document.querySelector(
              `[data-script-id="${scriptId}"] .mod-list-item-title`
            )?.innerText || "this mod";
          if (confirm(`Are you sure you want to uninstall ${modName}?`)) {
            ScriptManager.uninstallScript(scriptId);
            UI.switchTab("installed");
